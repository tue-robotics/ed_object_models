...
def from_yaml(yaml_path):
    ...
    for item in items:
        ...
        for path in model_paths:
            test_model_dir = os.path.join(path, item["type"])
        ...
        if not model_dir:
            rospy.logwarn("Couldn't find model directory of model type: '{}' in GAZEBO_MODEL_PATH".format(item["type"]))
            continue
        ...
        if not os.path.isfile(sdf_model_path):
            ...
            if sdf_list:
                ...
                rospy.logwarn("No sdf file was found for type: '{}'".format(item["type"]))
                continue
        ...
        # Spawn object
        outcome = spawn_model_srv.call(item["id"], sdf_file, "spawned_objects", object_pose, "world")
